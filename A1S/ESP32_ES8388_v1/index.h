...
<style>
*,html{font:normal 14pt verdana;color:#222}
body{background-color:#fafafa;padding:.25em;color:#fff}
.col{max-width:630px;margin:auto;text-align:center}
h3{margin:.3em 0}
button{border:none;box-shadow:1px 1px 1px 0px #666;border-radius:0.75em;padding: 0.6em 0.15em;}
...
.btns{margin:.25em auto;display:grid;grid-gap:.25em;grid-template-columns:repeat(auto-fit,minmax(120px,1fr))}
.btn-sm,.btns-sm,.sp{grid-template-columns:repeat(auto-fit,minmax(60px,1fr))}
.btns-bg{grid-template-columns:repeat(auto-fit,minmax(74px,1fr));margin-top:1em}
.btns.red button{background-color:#df513b;color:#fff}
.btns-sm button{font-size:.88rem}
.sp button{background:#e7e3c1;color:#0f2c4b;font-size:.8rem}
.ex{background:#19d191;color:#fff}
div#app span{width:3em;height:1.3em;background:#e7e3c1;color:navy;text-align:center;padding:.3em;margin:auto;border-radius:.5em}
